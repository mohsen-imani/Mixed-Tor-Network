...
#define SET_CUTOFF(target, msec) do {                       \
  ...
  TOR_LIST_FOREACH(next_circ, circuit_get_global_list(), head) {
    ...
    victim = next_circ;
    //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s:Circuit to check:%u:", get_options()->MYIP, (unsigned)next_circ->n_circ_id);
    ...
    //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s:Circuit to check:%u:it's still young, leave it alone", get_options()->MYIP, (unsigned)next_circ->n_circ_id);
    ...
    if (!any_opened_circs && victim->state != CIRCUIT_STATE_OPEN) {
      ...
      //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s:Circuit to check:%u:It's still young enough that we wouldn't close it, right? any_opened_circs:%d ", get_options()->MYIP, (unsigned)next_circ->n_circ_id, any_opened_circs);
      if (timercmp(&victim->timestamp_began, &close_cutoff, >)) {
        ...
        //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s:Circuit to check:%u:It's still young enough that we wouldn't close it, right? any_opened_circs:%d: skipped", get_options()->MYIP, (unsigned)next_circ->n_circ_id, any_opened_circs);
        continue;
      } else {
    ...
    //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s:Circuit to check:%u:if circ is !open, or if it's open but purpose is a non-finished (purpose %d, %s)", get_options()->MYIP, (unsigned)next_circ->n_circ_id, victim->purpose, circuit_purpose_to_string(victim->purpose));
    ...
    //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s:Circuit to check:%u:circuit not open, consider recording failure as timeout (purpose %d, %s)", get_options()->MYIP, (unsigned)next_circ->n_circ_id, victim->purpose, circuit_purpose_to_string(victim->purpose));
      ...
      if (circuit_timeout_want_to_count_circ(TO_ORIGIN_CIRCUIT(victim)) &&
         ...
         * Switch their purpose and wait. */
         //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s:Circuit to check:%u:Circuits are allowed to last longer for measurement (purpose %d, %s)", get_options()->MYIP, (unsigned)next_circ->n_circ_id, victim->purpose, circuit_purpose_to_string(victim->purpose));
        if (victim->purpose != CIRCUIT_PURPOSE_C_MEASURE_TIMEOUT) {
           ...
           * already "relaxed" circuits, which are counted above. */
           //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s:Circuit to check:%u:Record this failure to check for too many timeouts (purpose %d, %s)", get_options()->MYIP, (unsigned)next_circ->n_circ_id, victim->purpose, circuit_purpose_to_string(victim->purpose));
          if (!TO_ORIGIN_CIRCUIT(victim)->relaxed_timeout) {
    ...
    //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s:Circuit to check:%u:If this is a hidden service client? (purpose %d, %s)", get_options()->MYIP, (unsigned)next_circ->n_circ_id, victim->purpose, circuit_purpose_to_string(victim->purpose));
      switch (victim->purpose) {
      case CIRCUIT_PURPOSE_C_REND_READY:
...
circuit_predict_and_launch_new(void)
{
  ...
  if (0 && num_rend_services() && num_uptime_internal < 3) {
...
{
  return circuit_launch_by_extend_info(purpose, NULL, flags, 0);
}
...
//    log_fn(LOG_INFO,"%d failures so far, not trying.",n_circuit_failures);
   ...
   * n_circuit_failures */
  return circuit_establish_circuit(purpose, extend_info, flags, expedite);
}
