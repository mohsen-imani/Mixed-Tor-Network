                           ...
                           cell_direction_t cell_direction)
{
  ...
  int reason;
    ...
    //iSec-------------
  circ->no_cell++;
  if ((get_options()->MYLOG))
	tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|RECEIVED|PASSING|%s|CIRC|%u|CELL|%u|MYID|%d|NO|%d|Dir|%s|DONE",get_options()->MYIP,tv.tv_sec,tv.tv_usec, get_options()->MYIP,(unsigned)circ->n_circ_id,(unsigned)cell->circ_id, circ->rand_no,circ->no_cell,(cell_direction == CELL_DIRECTION_OUT)? "O":"I");
  tor_assert(cell);
  tor_assert(circ);
...
#define EARLY_WARNING_INTERVAL 3600
      ...
      const node_t *pnode =  node_get_by_id(TO_OR_CIRCUIT(circ)->p_chan->identity_digest);
      char *desc = channel_get_canonical_remote_descr(TO_OR_CIRCUIT(circ)->p_chan);
    //iSec-------------
    struct timeval tv;
    gettimeofday(&tv,NULL);
    // |AT|%ld|%ld     tv.tv_sec,tv.tv_usec
    //iSec-------------
  	...
  	tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|RECEIVED|EXTEND|%s|%s|CIRC|%u|CHAN|%u|DONE",get_options()->MYIP,tv.tv_sec,tv.tv_usec,get_options()->MYIP,desc,(unsigned)TO_OR_CIRCUIT(circ)->p_circ_id,(unsigned)TO_OR_CIRCUIT(circ)->p_chan->global_identifier);
    ...
    case RELAY_COMMAND_EXTENDED2:
      ...
      //const node_t *nnode =  node_get_by_id(circ->n_chan->identity_digest);
      //char *ndesc = channel_get_canonical_remote_descr(circ->n_chan);
      //char *desc = channel_get_canonical_remote_descr(TO_OR_CIRCUIT(circ)->p_chan);
      //tor_log(LOG_NOTICE, LD_CONTROL,"NOT-iSec:IP:%s: RECEIVED:EXTENDED:%s:->:%s:DONE",get_options()->MYIP,ndesc,get_options()->MYIP);
                             ...
                             streamid_t fromstream)
{
...
#endif
  //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s: appendeding cell: (%d) to the queue", get_options()->MYIP,cell->command);
  int exitward;
  if (circ->marked_for_close)
    return;
  ...
  if (!channel_has_queued_writes(chan)) {
     ...
     */
    log_debug(LD_GENERAL, "Primed a buffer."); //isec: we should stop this, right?
