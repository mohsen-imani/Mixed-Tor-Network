...
static int circuit_deliver_create_cell(circuit_t *circ,
                                       ...
                                       int relayed, circid_t oldid);
static int onion_pick_cpath_exit(origin_circuit_t *circ, extend_info_t *exit);
static crypt_path_t *onion_next_hop_in_cpath(crypt_path_t *cpath);
char *my_next_node_in_path(crypt_path_t *cpath);
char *my_pre_node_in_path(crypt_path_t *cpath);
static int onion_extend_cpath(origin_circuit_t *circ);
static int count_acceptable_nodes(smartlist_t *routers);
static int onion_append_hop(crypt_path_t **head_ptr, extend_info_t *choice);
int circuit_extend_isec(cell_t *cell, circuit_t *circ);
...
//isec-------------------------------------------------------------------------------
//insert a sample to the linked list
#include<stdio.h>
#include<stdlib.h>
#include <string.h>
struct Node* global_head = NULL;
int I_AM_ON_IT = 0;
int object_counter = 0; // count the number of class_isec object we created

void Insert(struct Node** head, class_isec *x)
{
...
void Delete(struct Node** head, char *ip)
{
	if (*head == NULL){
		return;
	}
	...
	if ((temp2 == *head) && (temp1 == NULL) && (equal == 1)){
		// single element in the list
		free((*head)->data->create_cell);
		free((*head)->data->ip);
		//free((*head)->data->digest);
		free(*head);
		*head = NULL;
		return;
	}else{
		if (equal == 1) 
		{
			...
			free((temp2)->data->create_cell);
			free((temp2)->data->ip);
			//free((temp2)->data->digest);
			free(temp2);
			return;
		}else{
			return;
...
void delete_by_id(struct Node** head, int id)
{
	if (*head == NULL){
		return;
	}
	...
	struct Node* temp1  = NULL;
	int equal = 0;
	//tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|wanna delete the node by id:%d", get_options()->MYIP,id);
	while(temp2 != NULL)
	{
		//tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|node id:%d", get_options()->MYIP,temp2->data->objectID);
		if  (temp2->data->objectID!= id){
			temp1 = temp2;
			temp2 = temp2->next;
		}else{
			equal = 1;
			//tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|found BREAK", get_options()->MYIP);
			break;
		}
	}
	if ((temp2 == *head) && (temp1 == NULL) && (equal == 1)){
		...
		free(*head);
		*head = temp1;
		//tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|linked list size*: %d", get_options()->MYIP,get_llist_length(head));
		return;
	}else{
		if (equal == 1) 
		{
...
class_isec* Findme(struct Node** head, char *ip)
{
	if (*head == NULL){
		return NULL;
	}
...
class_isec* find_me_by_id(struct Node** head, int id)
{
	if (*head == NULL){
		return NULL;
	}
...
int get_object_numbers(struct Node** head, char* ip)
{
	int l = 0;
	if (*head == NULL){
		return 0;
	...
	while(temp2 != NULL)
	{
		if (strcmp(ip, temp2->data->ip) == 0) l++;
		temp2 = temp2->next;
...
void print_llist(struct Node** head) {
	...
	// |AT|%ld|%ld     tv.tv_sec,tv.tv_usec
	//iSec-------------
	while(temp != NULL) {
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|IN-LLIST|CNTR|%d|IP|%s|circid|%u|OID|%d", get_options()->MYIP,tv.tv_sec,tv.tv_usec,temp->data->cntr,temp->data->ip,temp->data->oldid,temp->data->objectID );
		temp = temp->next;
		local_cnt++;
	}
...
// get the head of linked list and send the create cell till we have no more pendding create cells
int send_pendding_create_cells(struct Node** head, char *str)
{
	struct Node* temp = *head;
	if (I_AM_ON_IT == 1){ 
		//tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|%s|WORKING-ON-CCELL|Working|", get_options()->MYIP,str);
		return 1;
	}
	I_AM_ON_IT = 1;
	//tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|%s|Flag set to 1", str,get_options()->MYIP);
	//iSec-------------
	struct timeval tv;
	gettimeofday(&tv,NULL);
	// |AT|%ld|%ld     tv.tv_sec,tv.tv_usec
	//iSec-------------
	while((get_llist_length(head) != 0) && (*head != NULL))
	{
		class_isec *data = (*head)->data;
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|%s|WORKING-ON-CCELL|CNTR|%d|IP|%s|OID|%d|circID|%u|HEAD|%s|IN|", get_options()->MYIP,tv.tv_sec,tv.tv_usec,str,data->cntr,data->ip,data->objectID,data->oldid,(*head == NULL)? "NULL":"VALID");


		 if (check_hashmap_isec(data->p_chan, data->oldid) > 0){

			temp = *head;
			*head = (*head)->next;
			if (circuit_extend_isec(data->cell, data->circ) < 0){
				tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|%s|WORKING-ON-CCELL|BREAK?|We continued", get_options()->MYIP,tv.tv_sec,tv.tv_usec,str);
		    		//return -1;
			}
			delete_by_id(head, data->objectID);//free(temp);
			//tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|%s|WORKING-ON-CCELL|OUT|", get_options()->MYIP,str);
		}else{
			delete_by_id(head, data->objectID);//free(temp);
			tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s|AT|%ld|%ld: INCLALLBACK:IT IS NOT ON THE LIST:ID:%d:Timeout:%d:Sec:%d:Micro:%d", get_options()->MYIP,tv.tv_sec,tv.tv_usec, data->cntr, data->timeout, data->sec, data->micro);	
		}
	}
	I_AM_ON_IT = 0;
	//tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|%s|Flag set to 0",str, get_options()->MYIP);
	return 1;
...
int send_pendding_create_cells_for_given_ip(struct Node** head, char *ip, char *str)
{
	struct Node* temp = *head;
	//tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|%s|Reach to the cap for IP:%s |number of objects:%d", get_options()->MYIP,str, ip,get_object_numbers(head, ip));
	if (I_AM_ON_IT == 1){ 
		//tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|%s|WORKING-ON-CCELL|Working|", get_options()->MYIP,str);
		return 1;
	}
	...
	// |AT|%ld|%ld     tv.tv_sec,tv.tv_usec
	//iSec-------------
	while(get_object_numbers(head, ip) != 0)
	{
		class_isec *data = Findme(head, ip);
		if (data == NULL){
			tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|%s|could not find the object, continue", get_options()->MYIP,tv.tv_sec,tv.tv_usec,str);
			continue;
		}
		//tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|%s|object found, it's ID is:%d|IP|%s|", get_options()->MYIP,str,data->objectID, data->ip);
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|%s|WORKING-ON-CCELL|CNTR|%d|IP|%s|OID|%d|circID|%u|HEAD|%s|IN|", get_options()->MYIP,tv.tv_sec,tv.tv_usec,str,data->cntr,data->ip,data->objectID,data->oldid,(*head == NULL)? "NULL":"VALID");
		if (check_hashmap_isec(data->p_chan, data->oldid) > 0){
			if (circuit_extend_isec(data->cell, data->circ) < 0){
				tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|%s|WORKING-ON-CCELL|BREAK|we continued", get_options()->MYIP,tv.tv_sec,tv.tv_usec,str);
			...
			delete_by_id(head, data->objectID);
		}else{
			delete_by_id(head, data->objectID);
			tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s|AT|%ld|%ld:INCLALLBACK:IT IS NOT ON THE LIST:ID:%d:Timeout:%d:Sec:%d:Micro:%d", get_options()->MYIP,tv.tv_sec,tv.tv_usec, data->cntr, data->timeout, data->sec, data->micro);	
		}
 ...
 */
origin_circuit_t *
circuit_establish_circuit(uint8_t purpose, extend_info_t *exit, int flags, int expedite)
{
 ...
 //iSec:-------------------------------------------------
	...
	// |AT|%ld|%ld     tv.tv_sec,tv.tv_usec
//iSec-------------
  circ->base_.expedite = expedite;
  circ->base_.mypurpose = (int)purpose;
  if (circ  && circ->build_state->onehop_tunnel == 0){
     tor_log(LOG_NOTICE, LD_CONTROL,"iSec|%s|AT|%ld|%ld|INT|%d|ALL-IN-ONE-CREATE|ID|%u|G|%s|M|%s|E|%s|MYID|%d|EXP|%d|PPS|%d|Done",get_options()->MYIP,tv.tv_sec,tv.tv_usec, circ->build_state->is_internal,(unsigned)circ->base_.n_circ_id,circ->base_.entry_ip,circ->base_.middle_ip,circ->base_.exit_ip,circ->base_.rand_no,circ->base_.expedite,circ->base_.mypurpose);
  }
 //iSec:-------------------------------------------------
  return circ;
}
...
circuit_handle_first_hop(origin_circuit_t *circ)
{
  ...
  log_info(LD_CIRC,"is one hop?:%d Looking for firsthop '%s'",(int)circ->build_state->onehop_tunnel,
...
circuit_n_chan_done(channel_t *chan, int status)
{
  ...
  SMARTLIST_FOREACH_BEGIN(pending_circs, circuit_t *, circ)
	...
	circid_t mycirc = circ->n_circ_id; //isec
	if (CIRCUIT_IS_ORCIRC(circ)){
		mycirc = TO_OR_CIRCUIT(circ)->p_circ_id;
	}



        if (circuit_deliver_create_cell(circ, circ->n_chan_create_cell, 1, mycirc)<0) {
                            ...
                            int relayed, circid_t oldid)
{
  ...
  //if (get_options()->MYLOG == 1)
	//iSec-------------
	struct timeval tv;
	gettimeofday(&tv,NULL);
	// |AT|%ld|%ld     tv.tv_sec,tv.tv_usec
//iSec-------------
  tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|NEWID|%u|OLDID|%u|",get_options()->MYIP,tv.tv_sec,tv.tv_usec, (unsigned)id, (unsigned)oldid);
  circuit_set_n_circid_chan(circ, id, circ->n_chan);
  cell.circ_id = circ->n_circ_id;
  append_cell_to_circuit_queue(circ, circ->n_chan, &cell,
                               CELL_DIRECTION_OUT, 0);

  const node_t *pnode = node_get_by_id(circ->n_chan->identity_digest);
  char *desc = channel_get_canonical_remote_descr(circ->n_chan);
  if (get_options()->MYLOG == 1)
  tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|SENT|CREATE|%s|%s|CIRC|%u|CHAN|%u|DONE",get_options()->MYIP,tv.tv_sec,tv.tv_usec,get_options()->MYIP, (desc) ? desc : (pnode? pnode->node_ip : "NULL"), (unsigned)circ->n_circ_id, (unsigned)circ->n_chan->global_identifier);
...
circuit_send_next_onion_skin(origin_circuit_t *circ)
{
  ...
  if (circ->cpath->state == CPATH_STATE_CLOSED) {
	...
	circid_t mycirc = circ->base_.n_circ_id; //isec
	if (CIRCUIT_IS_ORCIRC(circ)){
		mycirc = TO_OR_CIRCUIT(circ)->p_circ_id;
	}


    if (circuit_deliver_create_cell(TO_CIRCUIT(circ), &cc, 0, mycirc) < 0)
    ...
    if (!hop) {
      ...
      if (circuit_timeout_want_to_count_circ(circ)) {
        ...
        long diff;
        tor_gettimeofday(&e);
        diff = tv_mdiff(&circ->base_.timestamp_began, &e);
      log_info(LD_CIRC,"circuit built!");
       ...
       tor_log(LOG_NOTICE, LD_CONTROL,"iSec|%s|AT|%ld|%ld|INT|%d|CREATE-BUILT|ID|%u|G|%s|M|%s|E|%s|MYID|%d|TIME|%ld|EXP|%d|PPS|%d|Done",get_options()->MYIP, tv.tv_sec,tv.tv_usec,circ->build_state->is_internal,(unsigned)circ->base_.n_circ_id,circ->base_.entry_ip,circ->base_.middle_ip,circ->base_.exit_ip,circ->base_.rand_no,diff,circ->base_.expedite, circ->base_.mypurpose);
    ...
    ec.create_cell.handshake_len = len;
     ...
     tor_addr_to_str(new_addr, (&hop->extend_info->addr), sizeof(new_addr), 1);
     const node_t *pnode;
     pnode = node_get_by_id(hop->prev->extend_info->identity_digest);
    
    //tor_log(LOG_NOTICE, LD_CONTROL,"NOT-iSec:IP:%s:SENT:EXTEND:%s:->:%s:Done",get_options()->MYIP,pnode->node_ip,new_addr );
...
//isec-Function that we call it every PERIODIC_TIMEOUT secondso check the pending extend cells
void mycallback_function_periodic(int fd, short event, void *arg)
{
  char *data = arg;
  if(get_options()->DEFENSE == 2 || get_options()->DEFENSE == 3){
	  if (send_pendding_create_cells(&global_head, "TIMEOUT") < 0)
		return;
	  return;
  }
  	//iSec-------------
	...
	// |AT|%ld|%ld     tv.tv_sec,tv.tv_usec
	//iSec-------------
  if(get_options()->DEFENSE == 99992){
	if (I_AM_ON_IT == 1) return;
	I_AM_ON_IT = 1;
	struct Node* temp = global_head;
	tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|TIMEOUT|Timer is up, send all the create cell", get_options()->MYIP,tv.tv_sec,tv.tv_usec);
	while((get_llist_length(&global_head) != 0) && (global_head != NULL))
	{
		class_isec *data = (global_head)->data;
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|TIMEOUT|WORKING-ON-NODE-LIST|CNTR|%d|IP|%s|OID|%d|", get_options()->MYIP,tv.tv_sec,tv.tv_usec,data->cntr,data->ip,data->objectID);
		temp = global_head;
		global_head = (global_head)->next;
		//now get the node
		
		if (temp->data->head == NULL){
			tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|TIMEOUT|On Objec %d in global list, the linked list is empty", get_options()->MYIP,tv.tv_sec,tv.tv_usec,temp->data->objectID);
		}else{
			tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|TIMEOUT|send out create cell on object %d in global list", get_options()->MYIP,tv.tv_sec,tv.tv_usec,temp->data->objectID);
			if (send_pendding_create_cells(&(temp->data->head), "TIMEOUT") < 0)
				return;
			free(temp->data->ip);
			free(temp->data);
			free(temp);
	...
	I_AM_ON_IT = 0;
	tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|TIMEOUT|Going out of call back function", get_options()->MYIP,tv.tv_sec,tv.tv_usec);
	return;
...
void mycallback_function(int fd, short event, void *arg)
{
  class_isec *hello = arg;
  (void)fd;
  (void)event;
  if (check_hashmap_isec(hello->p_chan, hello->oldid)< 0){
	tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s: INCLALLBACK:IT IS NOT ON THE LIST:ID:%d:Timeout:%d:Sec:%d:Micro:%d", get_options()->MYIP, hello->cntr, hello->timeout, hello->sec, hello->micro);	
	return;
  }
  tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s: INCLALLBACK:ID:%d:Timeout:%d:Sec:%d:Micro:%d", get_options()->MYIP, hello->cntr, hello->timeout, hello->sec, hello->micro);
   if (circuit_extend_isec(hello->cell, hello->circ) < 0)
    return;
...
circuit_extend(cell_t *cell, circuit_t *circ)
{
  ...
  const char *msg = NULL;
  int should_launch = 0;
  circid_t oldid = TO_OR_CIRCUIT(circ)->p_circ_id;
  if (circ->n_chan) {
  ...
  //char *iptest = fmt_addr_isec(&ec.orport_ipv4.addr);
  //tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|IP test*|%s|",get_options()->MYIP, iptest);



  if (get_options()->DEFENSE == 1)
  { //extend the circuit afer a timeout
	  class_isec *phello;
	  phello = tor_malloc_zero(sizeof(class_isec));
	  phello->cntr = rand();
	  phello->circ = circ;
	  phello->relayed = 1;
	  phello->oldid = oldid;

	  phello->p_chan = TO_OR_CIRCUIT(circ)->p_chan;
	  phello->cell = tor_malloc_zero(sizeof(cell_t));
	  phello->cell->circ_id = cell->circ_id;
	  phello->cell->command = cell->command;
	  memcpy(phello->cell->payload , cell->payload , CELL_PAYLOAD_SIZE);


	  phello->ip = strdup(fmt_addr_isec(&ec.orport_ipv4.addr));
	  int mytime = (rand() % get_options()->TIMEOUT_ISEC) + 1;
	  const int mysec = mytime/1000000;
	  const int myMicroSec = mytime -  mysec * 1000000;
	  phello->timeout = mytime;
	  phello->sec = mysec;
	  phello->micro = myMicroSec;
  	//iSec-------------
	struct timeval tv;
	gettimeofday(&tv,NULL);
	// |AT|%ld|%ld     tv.tv_sec,tv.tv_usec
	//iSec-------------
	  struct timeval poll_interval_tv = {mysec, myMicroSec};
	  tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|TIMER SET|ID|%d|Timeout|%d|Sec|%d|Micro|%d|TO IP|%s|hash|%d|", get_options()->MYIP,tv.tv_sec,tv.tv_usec, phello->cntr, mytime, mysec, myMicroSec,phello->ip, check_hashmap_isec(phello->p_chan,phello->oldid) );
	  circ->myevent = tor_event_new(tor_libevent_get_base(), -1,EV_TIMEOUT,
		                    mycallback_function, phello);
	  evtimer_add(circ->myevent, &poll_interval_tv);

  } else if (get_options()->DEFENSE == 2){ // normal Tor behavior
	  //create an object of out class
	  class_isec *phello;
	  phello = tor_malloc_zero(sizeof(class_isec));
	  phello->circ = circ;
	  phello->objectID =  (int)rand();
	  ...
	  phello->oldid = oldid;
	  object_counter++;
	  phello->cntr = object_counter;
	  Insert(&global_head,phello);
	  //print_llist(&global_head);
	  tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|ADD EXTEND CELL TO LLIST|ID|%d|TO IP|%s|LLIST-SIZE|%d|", get_options()->MYIP,tv.tv_sec,tv.tv_usec, phello->cntr, phello->ip,get_llist_length(&global_head));
	  
	  //if the number of received extend cells are more than a certain number send them quickly
	  if (get_llist_length(&global_head) >= get_options()->EXTEND_CAP)
	  {
		if (send_pendding_create_cells(&global_head, "CELL CAP") < 0)
			return -1;


	  }

   }else if(get_options()->DEFENSE == 3)
  {
	  class_isec *phello;
	  phello = tor_malloc_zero(sizeof(class_isec));
	  phello->circ = circ;
	  ...
	  Insert(&global_head,phello);
	  //print_llist(&global_head);
	  tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|ADD EXTEND CELL TO LLIST|ID|%d|TO IP|%s|LLIST-SIZE|%d|Object ID|%d|ON THIS IP|%d|", get_options()->MYIP,tv.tv_sec,tv.tv_usec, phello->cntr, phello->ip,get_llist_length(&global_head),phello->objectID,get_object_numbers(&global_head, phello->ip));
	  //tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|EXTEND CELL ON THIS IP:%d|ID|%d|TO IP|%s|LLIST-SIZE|%d|Object ID|%d|", get_options()->MYIP,get_object_numbers(&global_head, phello->ip), phello->cntr, phello->ip,get_llist_length(&global_head),phello->objectID);
	  
	  //if the number of received extend cells are more than a certain number send them quickly
	  if (get_object_numbers(&global_head, phello->ip) >= get_options()->EXTEND_CAP)
	  {
		if (send_pendding_create_cells_for_given_ip(&global_head,phello->ip, "CELL CAP") < 0)
			return -1;
	...
	return circuit_extend_isec(cell, circ);
  }
...
int
circuit_extend_isec(cell_t *cell, circuit_t *circ)
{
  channel_t *n_chan;
  relay_header_t rh;
  extend_cell_t ec;
  ...
  //tor_log(LOG_NOTICE, LD_CONTROL, "iSec:%s:We want to sent the create cell", get_options()->MYIP);
  //check whether circuit is open or not?
  // even if it is marked for close go out
  if (circ->magic != OR_CIRCUIT_MAGIC) return -1;
  ...
  if (tor_memeq(ec.node_id,
    ...
    //tor_log(LOG_NOTICE, LD_CONTROL, "iSec:%s:Establish connection", get_options()->MYIP);
  ...
  log_debug(LD_CIRC,
	  ...
	  if (circuit_deliver_create_cell(circ, &ec.create_cell, 1, oldid) < 0)
	    return -1;
                 ...
                 const uint8_t *rend_circ_nonce)
{
  ...
  //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:Here 1 %s", hex_str(circ->base_.n_chan->identity_digest, DIGEST_LEN));
  const node_t *pnode =  node_get_by_id(circ->p_chan->identity_digest);
  char *desc = channel_get_canonical_remote_descr(circ->p_chan);
    if (get_options()->MYLOG == 1)
  tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|SENT|CREATED|%s|%s|CIRC|%u|CHAN|%u|DONE",get_options()->MYIP,tv.tv_sec,tv.tv_usec,get_options()->MYIP, (desc) ? desc : (pnode? pnode->node_ip : "NULL"),(unsigned) circ->p_circ_id,(unsigned)9898 );
...
onion_pick_cpath_exit(origin_circuit_t *circ, extend_info_t *exit)
{
  ...
  log_debug(LD_CIRC, "iSec: cirucit is%d with length %d",state->is_internal,state->desired_path_len);
...
int
circuit_extend_garbage(cell_t *cell, circuit_t *circ)
{
  channel_t *n_chan;
  relay_header_t rh;
  extend_cell_t ec;
  ...
  { //extend the circuit afer a timeout
	  ...
	  phello->cntr = rand();
	  phello->circ = circ;
	  phello->create_cell = tor_memdup(&ec.create_cell,
		                                  sizeof(ec.create_cell));
	  phello->relayed = 1;
	  phello->oldid = oldid;
	  ...
	  phello->micro = myMicroSec;
	  struct timeval poll_interval_tv = {mysec, myMicroSec};
	  tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s:TIMER SET:ID:%d:Timeout:%d:Sec:%d:Micro:%d", get_options()->MYIP, phello->cntr, mytime, mysec, myMicroSec);
	  circ->myevent = tor_event_new(tor_libevent_get_base(), -1,EV_TIMEOUT,
		                    mycallback_function, phello);
	  evtimer_add(circ->myevent, &poll_interval_tv);
	  ...
	  object_counter++;
	  phello->cntr = object_counter;
	  phello->ip = channel_get_canonical_remote_descr(TO_OR_CIRCUIT(circ)->p_chan);
	  tor_addr_t remote_addr;
	  int have_remote_addr;
	  have_remote_addr = channel_get_addr_if_possible(circ->n_chan, &remote_addr);
	  phello->ip = "NULL";
	  if (have_remote_addr) {
	    phello->ip = tor_dup_addr(&remote_addr);
	  }  
	  Insert(&global_head,phello);
	  print_llist(&global_head);
		//iSec-------------
  ...
  {
  /*
	//lets first get the next node on the circuit
	//char *myid = tor_strdup(hex_str(ec.node_id, DIGEST_LEN));
	node_t *mynode = node_get_by_id(ec.node_id);
	if (mynode == NULL || mynode->llist_size != 1) 
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|OUT PUT:%s:|", get_options()->MYIP, (mynode == NULL)? "1":"0");
	if (mynode == NULL){
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|EXTEND TO NODE|NODE NULL 0|", get_options()->MYIP);
		if (circuit_deliver_create_cell(circ, &ec.create_cell, 1, oldid) < 0)
            		return -1;
		return 0;
	}
	tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|EXTEND TO NODE|%s|", get_options()->MYIP, mynode->node_ip);
	

	//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	//Now we have to consider two sets of linked list, one on each node, and one global linked list
	//first  add the extend cell to the node's linked list
          class_isec *phello;
          phello = tor_malloc_zero(sizeof(class_isec));
          phello->circ = circ;
          phello->create_cell = tor_memdup(&ec.create_cell,
          ...
          object_counter++;
          phello->cntr = object_counter;
          //phello->ip = channel_get_canonical_remote_descr(TO_OR_CIRCUIT(circ)->p_chan);// remove this from over statement
	  ...
	  if (phello->ip == NULL){
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|EXTEND TO NODE 2|NODE NULL|", get_options()->MYIP);
		if (circuit_deliver_create_cell(circ, &ec.create_cell, 1, oldid) < 0)
            		return -1;
		free(phello->create_cell);
		free(phello->ip);
		free(phello);
		return 0;

	  }
          Insert(&mynode->head,phello);
          print_llist(&mynode->head);
          tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|ADD EXTEND CELL TO LLIST|ID|%d|TO IP|%s|LLIST-SIZE|%d|oldid|%u|", get_options()->MYIP, phello->cntr, phello->ip,get_llist_length(&mynode->head), phello->oldid);
	//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

         // Now check this node have enough pendding create cell or not, if Yes, then send them out
          if (get_llist_length(&mynode->head) >= get_options()->EXTEND_CAP)
          {
                if (send_pendding_create_cells(&mynode->head, "CELL CAP") < 0)
                        return -1;

		//then we should remove this node from global linked list
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|CREATE CELL SENT OUT|ID|%d|TO IP|%s|LLIST-SIZE|%d|", get_options()->MYIP, object_counter, mynode->node_ip,get_llist_length(&global_head));
		Delete(&global_head, phello->ip);
                tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|REMOVE NODE|ID|%d|TO IP|%s|LLIST-SIZE|%d|", get_options()->MYIP, object_counter, mynode->node_ip,get_llist_length(&global_head));
          }else{
		// add the node to the global linked list
		// do we have it one the list?
		if (Findme(&global_head,mynode->node_ip) == NULL){
			// we do not have it, then add it
		        class_isec *hello;
          		hello = tor_malloc_zero(sizeof(class_isec));
	                tor_addr_t remote_address;
          	        int have_remote_address;
          	        have_remote_address = channel_get_addr_if_possible(circ->n_chan, &remote_address);
          		hello->ip = NULL;
			memcpy(hello->digest, mynode->identity, DIGEST_LEN);
          		if (have_remote_address) {
            			hello->ip = tor_dup_addr(&remote_address);
          		}  
			if (hello->ip == NULL){
                		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|EXTEND TO NODE|NODE NULL|", get_options()->MYIP);
                		if (circuit_deliver_create_cell(circ, &ec.create_cell, 1, oldid) < 0)
                        		return -1;
				free(hello->ip);
				//free(hello->digest);
				free(hello->create_cell);
				free(hello);
			}
			Insert(&global_head,hello);
		}
  ...
  */
	  ...
	  /*
	  tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|WE want to extend the circuit. DEFENSE == 3", get_options()->MYIP);
          class_isec *phello;
          phello = tor_malloc_zero(sizeof(class_isec));
          phello->circ = circ;
          phello->create_cell = tor_memdup(&ec.create_cell,
	  ...
	  if (phello->ip == NULL){
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|EXTEND TO NODE|NODE NULL|1|", get_options()->MYIP);
		if (circuit_deliver_create_cell(circ, &ec.create_cell, 1, oldid) < 0)
            		return -1;
		free(phello->create_cell);
		free(phello->ip);
	  ...
	  tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|Circuit extending to IP:%s", get_options()->MYIP, phello->ip);

	  // check whether we have this ip on out global linked list
	  class_isec *found = Findme(&global_head,phello->ip);
	  if (found == NULL)// add it this node to the global list and add the cell to its linked list
	  {
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|IP:%s Not found in the global linked list", get_options()->MYIP, phello->ip);
	        class_isec *nodeobj;
  		nodeobj = tor_malloc_zero(sizeof(class_isec));
                tor_addr_t remote_address;
  	        int have_remote_address;
  	        have_remote_address = channel_get_addr_if_possible(circ->n_chan, &remote_address);
  		nodeobj->ip = NULL;
		nodeobj->head = NULL;
		nodeobj->oldid = 1111;
		nodeobj->objectID = (int)rand();
  		if (have_remote_address) {
    			nodeobj->ip = tor_dup_addr(&remote_address);
  		}  
		if (nodeobj->ip == NULL){
        		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|EXTEND TO NODE|NODE NULL|2|", get_options()->MYIP);
        		if (circuit_deliver_create_cell(circ, &ec.create_cell, 1, oldid) < 0)
                		return -1;
			free(nodeobj->ip);
			//free(hello->digest);
			free(nodeobj->create_cell);
			free(nodeobj);
		}
		Insert(&global_head,nodeobj);
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|IP:%s\tID:%d added to the head of global linked list", get_options()->MYIP, phello->ip, nodeobj->objectID);
		found = global_head->data;
	  }

	  // now insert the data the node in the globle linked linst which is in 'found' 
	  print_llist(&(global_head));
	  Insert(&(found->head),phello);
	  tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|IP:%s\tEX OBJID:%d added extend cell to the head of node's linked list", get_options()->MYIP, phello->ip,phello->objectID);
	  print_llist(&(found->head));



	  //Now check the metrics for sending out the create cells

          if (get_llist_length(&(found->head)) >= get_options()->EXTEND_CAP && 0)
          {
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|IP:%s\tEX OBJID:%d We have enought create cell on the node in global list, send them out", get_options()->MYIP, found->ip, found->objectID);
                if (send_pendding_create_cells(&(found->head), "CELL CAP") < 0)
                        return -1;

		//then we should remove this node from global linked list
		tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|Now delete node (%s:%d) from the global list", get_options()->MYIP, found->ip,found->objectID);
		Delete(&global_head, found->ip);
                tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|Object removed from the global list", get_options()->MYIP);
	  }else{
	  	tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|IP:%s\tEX OBJID:%d We do NOT have enought create cell on the node in global list, wait", get_options()->MYIP, found->ip, found->objectID);
	  }
	*/
	  ...
	  if (get_object_numbers(&global_head, phello->ip) >= get_options()->EXTEND_CAP)
	  {
		if (send_pendding_create_cells_for_given_ip(&global_head,myip, "CELL CAP") < 0)
			return -1;
