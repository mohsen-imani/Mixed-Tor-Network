...
#define SELF_EXCLUDED_WARN_INTERVAL 3600
  ...
  if (test_or && (!orport_reachable || !circuit_enough_testing_circs())) {
    ...
    circuit_launch_by_extend_info(CIRCUIT_PURPOSE_TESTING, ei,
                            CIRCLAUNCH_NEED_CAPACITY|CIRCLAUNCH_IS_INTERNAL, 5);
    extend_info_free(ei);
  }
...
char *my_address_ip(){
  ...
  //tor_addr_t myaddr;
  //tor_addr_copy(&last_guessed_ip, &myaddr);
  /*char new_addr[TOR_ADDR_BUF_LEN];
  tor_addr_to_str(new_addr, (&last_guessed_ip), sizeof(new_addr), 1);
  strlcpy(source, new_addr, TOR_ADDR_BUF_LEN);
  return;*/
  char new_addr[TOR_ADDR_BUF_LEN];
  uint32_t *addr;
  if (router_guess_address_from_dir_headers(addr) < 0) {
  ...
  strlcpy(new_addr, fmt_addr32(*addr), TOR_ADDR_BUF_LEN);
...
const char *
iSec_node_description(const node_t *node)
{
