                                           ...
                                           bandwidth_weight_rule_t rule)
{
  ...
  //tor_log(LOG_NOTICE, LD_CONTROL,"@iSec: %d", smartlist_len(sl));
  {
                            ...
                            u64_dbl_t **bandwidths_out)
{
  ...
  log_info(LD_CIRC,"@iSec:%s:NODE-LIST-START-----------------------------------------",get_options()->MYIP);
  ...
  SMARTLIST_FOREACH_BEGIN(sl, const node_t *, node) {
    ...
    node_get_address_string(node, buf, INET_NTOA_BUF_LEN);
    log_info(LD_CIRC,"@iSec:%s:NODE:%d:IP:%s:B:%f:E?:%d:G?:%d:DIR?:%d:BW:%d:DONE",get_options()->MYIP,rule,buf, bandwidths[node_sl_idx].dbl,is_exit, is_guard, is_dir,this_bw);
  } SMARTLIST_FOREACH_END(node);
  log_info(LD_CIRC,"@iSec:%s:NODE-LIST-END-----------------------------------------",get_options()->MYIP);
