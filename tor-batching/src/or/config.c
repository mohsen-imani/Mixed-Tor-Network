...
static config_var_t option_vars_[] = {
  ...
  V(LearnCircuitBuildTimeout,    BOOL,     "0"),//isec we do not want to learn this
  ...
  V(MYIP,             STRING,   "129.107.211.96"),
  V(MYLOG,             BOOL,   "1"),
  V(DATACAP,UINT,     "1"),
  V(TIMEOUT_ISEC,          INT,   "1000000"),// timeout for sending the create cell after receiving the extend cell.
  V(PERIODIC_TIMEOUT,          INT,   "1000000"),// the timer we check our pending extend cells.
  V(DEFENSE,          INT,   "7"),//DEFENSE == 1, extenc circuits after random timeout, DEFENSE == 2 extends the cirucits when we have certain number of extend cells, DEFENSE == 3 extends circuits when we have certain number of extend cells towards a certain node
  V(EXTEND_CAP,          INT,   "10"), //number of extend cells that we extend circuits afterward
