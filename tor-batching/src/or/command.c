...
command_process_create_cell(cell_t *cell, channel_t *chan)
{
  ...
  //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:Here 1 %s", hex_str(circ->base_.n_chan->identity_digest, DIGEST_LEN));
    ...
    //iSec-------------
  const node_t *pnode =  node_get_by_id(chan->identity_digest);
  char *desc = channel_get_canonical_remote_descr(chan);
  if (get_options()->MYLOG == 1)
  	tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|RECEIVED|CREATE|%s|%s|CIRC|%u|CHAN|%u|DONE",get_options()->MYIP,tv.tv_sec,tv.tv_usec,get_options()->MYIP, (desc) ? desc : (pnode? pnode->node_ip : "NULL"),(unsigned)cell->circ_id, (unsigned)chan->global_identifier);
...
command_process_created_cell(cell_t *cell, channel_t *chan)
{
  ...
  char *desc = channel_get_canonical_remote_descr(chan);
  if (get_options()->MYLOG == 1)
	tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|RECEIVED|CREATED|%s|%s|CIRC|%u|CHAN|%u|DONE",get_options()->MYIP,tv.tv_sec,tv.tv_usec,get_options()->MYIP,desc,(unsigned)cell->circ_id,(unsigned)chan->global_identifier);
  if (CIRCUIT_IS_ORIGIN(circ)) { /* we're the OP. Handshake this. */
    ...
    if (get_options()->MYLOG == 1)
    tor_log(LOG_NOTICE, LD_CONTROL,"iSec|IP|%s|AT|%ld|%ld|SENT|EXTENDED|%s|%s|CIRC|%u|CHAN|%u|DONE",get_options()->MYIP,tv.tv_sec,tv.tv_usec, get_options()->MYIP,(desc) ? desc : (pnode? pnode->node_ip : "NULL"),(unsigned)cell->circ_id,(unsigned)chan->global_identifier);
