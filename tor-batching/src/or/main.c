...
static void second_elapsed_callback_isec(periodic_timer_t *timer, void *args);
...
run_scheduled_events(time_t now)
{
  ...
  const or_options_t *options = get_options();
  //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s: MOHSEN run_scheduled_events", get_options()->MYIP);
...
static periodic_timer_t *second_timer_isec = NULL;//isec
...
//isec-----------------------------------------------------
static void
run_scheduled_events_isec(time_t now){

  int s = 2 + 3;
}

static void second_elapsed_callback_isec(periodic_timer_t *timer, void *arg)
{
...
static void second_elapsed_callback(periodic_timer_t *timer, void *arg)
{
  /* XXXX This could be sensibly refactored into multiple callbacks, and we
   * could use Libevent's timers for this rather than checking the current
   * time against a bunch of timeouts every second. */
  //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s: MOHSEN second_elapsed_callback 0", get_options()->MYIP);
  static time_t current_second = 0;
  time_t now;
  ...
  //tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s: MOHSEN second_elapsed_callback 1", get_options()->MYIP);
...
do_main_loop(void)
{
   ...
   * TLS context. */
  log_err(LD_BUG,"MOHSEN Error initializing keys; exiting");
  ...
  /* set up once-a-second callback. */
  if (! second_timer_isec) {
    struct timeval one_second_isec;
    one_second_isec.tv_sec = 5;
    one_second_isec.tv_usec = 0;

    second_timer_isec = periodic_timer_new(tor_libevent_get_base(),
                                      &one_second_isec,
                                      second_elapsed_callback_isec,
                                      NULL);
    tor_assert(second_timer_isec);
  }

  tor_log(LOG_NOTICE, LD_CONTROL,"iSec:IP:%s: MOHSEN do_main_loop 1", get_options()->MYIP);
#ifndef USE_BUFFEREVENTS
  if (!refill_timer) {
...
log_notice(LD_GENERAL, "iSec:Tor started at:%ld:%ld", tv.tv_sec,tv.tv_usec);
//iSec-------------
  ...
  int mytime =  get_options()->PERIODIC_TIMEOUT;
  const int mysec = mytime/1000000;
  const int myMicroSec = mytime -  mysec * 1000000;
  struct timeval poll_interval_tv = {mysec, myMicroSec};
  struct event *myevent = tor_event_new(tor_libevent_get_base(), -1,EV_TIMEOUT|EV_PERSIST,
                            mycallback_function_periodic, (char *)"PERIODIC_TIMEOUT");
  evtimer_add(myevent, &poll_interval_tv);
...
tor_free_all(int postfork)
{
  ...
  periodic_timer_free(second_timer_isec);//isec
#ifndef USE_BUFFEREVENTS
